...
public class EvaluatePCRanked {
/**
 * Meant for taking freebaseDbpediaAppend and one of the f-db-*score*-ranked.txt files
 * (with a rank threshold pre-programmed in the mapper)
 * and producing the output:
 * <idy>[tab]<f-type>[tab]<d-type>[tab]<idx>
 * Note that all types <idy> are guaranteed to be at least the threshold rank.
 * EvaluatePairsCompleteness1 can then deduplicate the idxs so we can serially
 ...
 */
  public static class TokenizerMapper
    ...
    public void map(Object key, Text value, Context context
    	...
    	int rankThreshold=200; //let's start with this
  ...
  public static void main(String[] args) throws Exception {
    ...
    Job job = new Job(conf, "build pairs completeness file from ranked list");
    job.setJarByClass(EvaluatePCRanked.class);
