...
public class MatcherWithoutTypeBlocking {
	
	
	
/**
 * we will not be using type information at all here.
 * @author Mayank
 *
 */
  public static class TokenizerMapper
	  ...
	  static int bkvThreshold=5;	//do not assign more than this many bkvs/entity (even after controlling for types)
	  static int index=1;
    ...
    public void map(Object key, Text value, Context context
    	...
    	outer:
    		for(int i=lengths.size()-1; i>=0; i--)
    			...
    			{			
    				context.write(new Text(bkv), new Text(json));
  ...
  public static void main(String[] args) throws Exception {
    ...
    Job job = new Job(conf, "freebase-dbpedia: Matcher without type blocking");
  ...
  //  DistributedCache.addCacheFile(new URI(args[3]),job.getConfiguration());
    job.setJarByClass(MatcherWithoutTypeBlocking.class);
