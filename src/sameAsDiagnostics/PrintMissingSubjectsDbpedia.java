...
public class PrintMissingSubjectsDbpedia {
/**
 *This file is for diagnosing the approximately 1.3 million sameAs links in curated_freebase_links
 *that are missing. We will run against the dbpedia json-objects files
 *so make sure everything is set up correctly. If a dbpedia subject is missing in the json file,
 * but present in the sameAs file, we will
 *print out the link as:
 *<idx>[tab]<fr-instance>[tab]<db-instance>[tab]"dm"
 ...
 */
  public static class TokenizerMapper
    ...
    public void map(Object key, Text value, Context context
    	...
    	}else{//came from withTypes file: send flag that it's present
  ...
  public static class IntSumReducer
	  ...
	  public void reduce(Text key, Iterable<Text> values,
	     ...
	     for(String m: missing){
	    	 context.write(new Text(m), new Text("dm"));
	     }
  ...
  public static void main(String[] args) throws Exception {
    ...
    Job job = new Job(conf, "sameAs diagnostics: dbpedia");
    job.setJarByClass(PrintMissingSubjectsDbpedia.class);
